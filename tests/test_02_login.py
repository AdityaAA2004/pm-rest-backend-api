...
Requires ctx.state: email1, user1_token (from previous section).
"""
...
def run(ctx: TestContext) -> None:
    ...
    if ctx.assert_status(resp, 200, "Login user1 (valid credentials)"):
        ...
        if not ctx.no_sensitive_field_in(data, "password", "login response"):

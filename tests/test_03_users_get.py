"""
Section 3 — User: unauthenticated GET operations.
...
def run(ctx: TestContext) -> None:
    section("3 · USER — GET (unauthenticated reads)")
    ...
    if ctx.assert_status(resp, 200, "GET /api/users → 200"):
        ...
        if not ctx.no_sensitive_field_in(items, "password", "GET /api/users list"):
            ...
            ctx.ok("Password field absent from all records in list response")
    ...
    if ctx.assert_status(resp, 200, "GET /api/users?page=0 → normalised"):
        meta = ctx.safe_json(resp).get("meta", {})
    ...
    if ctx.assert_status(resp, 200, "GET /api/users?limit=9999 → capped"):
        meta = ctx.safe_json(resp).get("meta", {})
    ...
    if ctx.assert_status(resp, 200, "GET /api/users?page=99999 → empty"):
    ...
    if user1_id:
        ...
        if ctx.assert_status(resp, 200, f"GET /api/users/{user1_id} → 200"):
            ...
            if not ctx.no_sensitive_field_in(data, "password", f"GET /api/users/{user1_id}"):
                ctx.warn("Password field exposed in single-record response")
            else:
                ctx.ok("Password field absent from single-record response")
    ...
    # 3-10  Float ID → 400 or 404
    resp = ctx.req("GET", "/api/users/1.5")
    if resp.status_code in (400, 404):
        ctx.ok(f"GET /api/users/1.5 → HTTP {resp.status_code} (acceptable)")
    else:
